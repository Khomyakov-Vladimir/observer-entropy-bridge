...
**DOI (this version):** [10.5281/zenodo.20703033](https://doi.org/10.5281/zenodo.20703033)
...
Framework"* (version 3.24.12). The paper constructs a self-contained
...
## What Is New in Version 3.24.12

Version 3.24.12 is a **verification-strengthening release** that adds an
independent **symbolic (computer-algebra) verification track** alongside
the existing numerical one. **No theorem statement, numerical result, or
figure is changed**; the addition certifies the exact closed-form
identities of the manuscript as residuals identically equal to zero.

- **New symbolic verification script
  (`verify_bridge_symbolic_v3.24.12.py`).** A SymPy-based script that
  verifies the exact closed-form identities symbolically, with every
  residual decided exactly (rational/polynomial normal form), with **no
  reliance on the randomized Schwartz–Zippel branch**. Coverage:
  - **(A)** cosh algebraic identity (Lemma 6.23);
  - **(B)–(C)** Fisher matrices $I(\theta^*) =
    \mathrm{diag}(1,\tfrac12,\tfrac12)$ and
    $\mathrm{diag}(\tfrac32,\tfrac65,\tfrac25,\tfrac25)$
    (Lemma 6.22, Proposition 6.36);
  - **(D)–(E)** leading coefficients $\tfrac14(\alpha^2+\beta^2)$ and
    $\tfrac35\alpha^2+\tfrac15\beta^2+\tfrac15\gamma^2$
    (Propositions 6.24, 6.38), including independent
    $\varepsilon^2$-coefficient series checks;
  - **(F)** fibre-theoretic closed-form identity
    $S_\mathrm{obs}^\mathrm{fib} = \log N - H_\mathrm{w}$ (Theorem 11.9);
  - **(G)** the difference-Jacobian step of Lemma 6.48 (the contrast
    identity $\det M = w_1$, the factorization
    $I_\mathrm{bb} = J\,W\,J^\top$ with $W \succ 0$, and the resulting
    invertibility of the difference Jacobian $J$ for $m = 2, 3$).
  The script terminates with the verdict **`SYMBOLIC PASS (PROVEN)`**.
- **New symbolic verification log
  (`Results_verify_bridge_symbolic_v3.24.12.txt`).** Complete verbatim
  console output of the symbolic script, captured by the embedded
  `_Tee` logger, with all residuals shown identically zero.
- **Two new manuscript appendices** reproducing the symbolic script
  (Appendix C) and its log (Appendix D); the numerical script and log
  are renumbered to Appendices A and B.
- **Updated Reproducibility remark (Remark 6.25)** documenting the
  two independent verification scripts (numerical and symbolic).
- **Scripts renamed** `…_v3.24.3.py` → `…_v3.24.12.py` with consistent
  internal version strings.

**No change** to: the Bridge Theorem (Theorem 6.7), the Sufficient
Conditions Theorem (Theorem 6.43, conditions (A1)–(A4)), Lemma 6.48,
the Riemannian-geometric formulation, the Cognitive Uncertainty
Principle, the Landauer bound, the fibre-theoretic closed-form identity
(Theorem 11.9), numerical results, the empirical $C_*$ table, the
validation criterion, the off-center robustness checks, or
Figures 1–7 and the supplementary diagnostic figures.

---

## What Is New in Version 3.24.3

Version 3.24.3 is a **proof-strengthening release** that closes a
logical gap in the Sufficient Conditions Theorem by adding a fourth
checkable condition and supplies a fully self-contained proof of the
key vanishing lemma. **No theorem statement affecting the Bridge
Theorem, no numerical result, and no figure is changed.** The changes
concern the internal hypotheses and proofs of
Section 6 (Sufficient Conditions) and Section 11.

- **New condition (A4) — Global identifiability on the neutral
  locus — in the Partition-Adapted Parameterization
  (Definition 6.41).** The earlier three conditions (A1)–(A3) supply
  only *local* diffeomorphicity of the parametric map; (A4) adds the
  *global* injectivity of the restricted between-class map $\Phi_0$
  on the within-class neutral locus $\{\theta_\mathrm{w}=0\}$. This is
  needed to pin down the base point of the deformation
  ($\sigma_\mathrm{b}(\theta_\mathrm{b}^*,0)=\theta_\mathrm{b}^*$) and
  replaces the appeal to mere local injectivity used in v3.24.2.
- **Sufficient Conditions Theorem (Theorem 6.43) now rests on four
  checkable finite-dimensional conditions (A1)–(A4)** rather than
  three. The conclusion (that the Parametric Deformation Assumption
  is a theorem for partition-adapted families) and the deformation
  vector $v$ are unchanged.
- **Strengthened Lemma 6.48 (Vanishing of
  $D_\mathrm{w}\sigma_\mathrm{b}$ at zero).** The block-rank argument
  is rewritten using subadditivity of rank under column concatenation
  (avoiding any square-block determinant formula and any diagonality
  assumption on the between-class block), and the within-class
  insensitivity $C_\mathrm{within}=0$ is derived explicitly from
  (L1).
- **(A4) verified for both worked examples and for softmax families.**
  Propositions 6.52–6.53 (Examples 1 and 2) and Corollary 6.51
  (Softmax Families) now include explicit verification of (A4) via the
  minimal-exponential-family mean-parameter bijection (Brown 1986,
  Thm. 3.6).
- **Self-contained proof in Remark 11.\* (Identification of the Fisher
  form).** The reduction of the fibre expansion to the Bridge Theorem
  is now carried out explicitly via the law of total covariance for
  the score, replacing the v3.24.2 statement that omitted the
  (purely algebraic) verification.
- **Verification script renamed**
  `verify_bridge_theorem_v3.24.2.py` →
  `verify_bridge_theorem_v3.24.3.py` with consistent internal version
  strings.

**No change** to:

- the Bridge Theorem (Theorem 6.7), its quadratic scaling law, or its
  $O(\varepsilon^3)$ remainder;
- the Riemannian-geometric formulation, the Cognitive Uncertainty
...
2. **Sufficient Conditions Theorem (Theorem 6.43).** The
   ...
   (D1)–(D3)) is proved to be a consequence of four checkable
   finite-dimensional conditions — within-class neutrality (A1),
   lift closure (A2), immersion condition (A3), and global
   identifiability on the neutral locus (A4) — that hold
...
| 3.24.3 | [10.5281/zenodo.20638126](https://doi.org/10.5281/zenodo.20638126) | Proof-strengthening release. Added a fourth checkable condition (A4) — global identifiability on the within-class neutral locus — to the Partition-Adapted Parameterization (Def. 6.41); the Sufficient Conditions Theorem (Thm. 6.43) now rests on four conditions (A1)–(A4) rather than three, closing the local-to-global gap in determining the deformation base point. Rewrote Lemma 6.48 via subadditivity of rank (no square-block determinant formula, no diagonality assumption). (A4) verified for both worked examples and softmax families via the minimal-exponential-family mean-parameter bijection (Brown 1986). Self-contained identification of the fibre-Fisher form with $v^\top I v$ via the law of total covariance. Script renamed to `verify_bridge_theorem_v3.24.3.py`. No change to the Bridge Theorem, numerical results, the empirical $C_*$ table, figures, or verification output. |
| **3.24.12** | **[10.5281/zenodo.20703033](https://doi.org/10.5281/zenodo.20703033)** | **Verification-strengthening release. Added an independent symbolic (computer-algebra) verification track: new script `verify_bridge_symbolic_v3.24.12.py` (SymPy) certifying the exact closed-form identities — cosh identity (Lemma 6.23), Fisher matrices (Lemma 6.22, Prop. 6.36), leading coefficients (Props. 6.24, 6.38), fibre identity (Thm. 11.9), and the difference-Jacobian step of Lemma 6.48 ($\det M = w_1$, $I_\mathrm{bb}=J\,W\,J^\top$ with $W\succ0$, invertibility of $J$ for $m=2,3$) — as residuals identically zero, with verdict `SYMBOLIC PASS (PROVEN)` and no reliance on the randomized Schwartz–Zippel branch. New symbolic log `Results_verify_bridge_symbolic_v3.24.12.txt`. Two new manuscript appendices (symbolic script and log); numerical scripts renamed to `…_v3.24.12.py`. No change to any theorem, numerical result, figure, or the empirical $C_*$ analysis.** |
...
```
observer_entropy_bridge_theorem_v3.24.12/
├── README.md
├── LICENSE
├── requirements.txt
├── scripts/
│   ├── verify_bridge_theorem_v3.24.12.py
│   └── verify_bridge_symbolic_v3.24.12.py
├── Results_verify_bridge_theorem_v3.24.12.txt
├── Results_verify_bridge_symbolic_v3.24.12.txt
└── figures/
...
Theorem 11.9. The numerical verification script
`verify_bridge_theorem_v3.24.12.py` includes all checks from v3 plus
...
seed `OFFCENTER_SEED = 42`. The symbolic verification script
`verify_bridge_symbolic_v3.24.12.py` independently certifies the
exact closed-form identities (blocks (A)–(G)) as residuals
identically zero, terminating with `SYMBOLIC PASS (PROVEN)`.
...
Symbolic verification additionally requires `sympy` (see
`requirements.txt`).
...
```bash
python scripts/verify_bridge_theorem_v3.24.12.py
```

To run the independent symbolic (computer-algebra) verification:

```bash
python scripts/verify_bridge_symbolic_v3.24.12.py
```

The numerical script reproduces all figures and the
$O(\varepsilon^3)$ asymptotic checks; the symbolic script verifies the
exact closed-form identities and prints `SYMBOLIC PASS (PROVEN)`.
...
A Minimal Information-Theoretic Framework*, Version 3.24.12 (2026).
DOI: [10.5281/zenodo.20703033](https://doi.org/10.5281/zenodo.20703033)
